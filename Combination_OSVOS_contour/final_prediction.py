...
def compute_combo_img(translation_pred, OSVOS_img):
    contour_img = np.zeros(OSVOS_img.shape).astype(np.uint8)
    translation_pred = np.expand_dims(translation_pred.detach().numpy().astype(np.int32), axis=0)
    contour_img = cv2.fillPoly(contour_img, translation_pred, color=(255, 255, 255))
...
def plot_all(translation_pred, OSVOS_img):
    contour_img = np.zeros(OSVOS_img.shape[0:2]).astype(np.uint8)
    translation_pred = np.expand_dims(translation_pred.detach().numpy().astype(np.int32), axis=0)
    contour_img = cv2.fillPoly(contour_img, translation_pred, color=255)
